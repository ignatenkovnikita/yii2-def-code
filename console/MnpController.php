...
class MnpController extends Controller
{
    ...
    public function actionDownloadDetail()
    {
        ...
        $url = str_replace('{login}', $this->moduleDefCode->smscLogin, $url);
        $url = str_replace('{password}', $this->moduleDefCode->smscPassword, $url);
        $url = str_replace('{date}', date('d.m.Y', strtotime("-1 days")), $url);

        $name = 'mnp_detail';
        $fileName = $this->basePath . $name . '.text';
        $this->log('download ' . $fileName);
        if (file_exists($fileName)) {
            $newFileName = $this->basePath . $name . '_' . date('Y-m-d-H-i-s', filemtime($fileName)) . '.csv';
            rename($fileName, $newFileName);
    ...
    public function actionImportDetail()
    {
        ...
        $fileName = $this->basePath . 'mnp_detail.text';
        if (file_exists($fileName) && filesize($fileName)) {
            $this->log('start file ' . $fileName);
            ...
            while (!feof($fn)) {
                ...
                if ($matches) {
                    ...
                    if ($defMncMcc) {
                        $defMnp = new DefMnp();
                        $defMnp->phone = $phone;
                        $defMnp->def_mnc_mcc_id = $defMncMcc->id;
                        if (!$defMnp->save()) {
                            $this->log('error save defMnp erros ' . print_r($defMnp->errors, true));
                        }
                    } else {
                        $this->log('not found  mcc - ' . $mcc . ', mnc - ' . $mnc);
                    }
                }
            }
